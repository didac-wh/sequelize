          ...
          return Promise.all([
            ...
            .then(tableInfos => {
              ...
              _.each(attributes, (columnDesc, columnName) => {
                if (!columns[columnName] && !columns[decamelize(columnName)]) {
                  const currentAttributes = attributes[columnName] || attributes[camelize(columnName)];
                  changes.push(() => this.QueryInterface.addColumn(this.getTableName(options), decamelize(columnName), currentAttributes));
                }
              });
              _.each(columns, (columnDesc, columnName) => {
                const currentAttributes = attributes[columnName] || attributes[camelize(columnName)];
                if (!currentAttributes) {
                  changes.push(() => this.QueryInterface.removeColumn(this.getTableName(options), decamelize(columnName), options));
                  ...
                  changes.push(() => this.QueryInterface.changeColumn(this.getTableName(options), decamelize(columnName), currentAttributes));
                }
              });
